...
page(1, 'SceneLab v1.2\n使用说明', '光学动捕相机部署与理论精度仿真工作台')
story.extend([SceneGraphic(),Spacer(1,18)])
add('从场地到方案，用空间分析辅助部署决策。', 'h2')
add('本指南面向首次使用的同事。软件在本机运行，内置相机参数库、三维场景编辑、覆盖与理论精度分析，并可输出技术报告。')
table(['从这里开始','页码'],[
    ['安装与第一次运行','02'],['认识界面，设置场地','03'],['相机库与相机部署','04'],['对象编辑与 Marker 尺寸','05'],['运行分析与读取热图','06'],['逐像素查看相机成像','07'],['技术报告、保存与交接','08'],['常见问题与计算边界','09'],['方管与 P3 IR','10'],['多选、分组与右键操作','11'],
],[410,73])
add('版本 1.2.0 · Windows x64 · 发布日期 2026-09-22', 'small')

page(2, '安装与第一次运行', '选择安装版或免安装版即可。两者功能一致，均自带运行环境，不需要安装 Node.js，也不需要联网登录。')
h('方式 A / 免安装版')
add('1. 将 SceneLab-1.2.0-Windows-x64.zip <b>完整解压</b>到普通文件夹。<br/>2. 打开该文件夹，双击 <b>SceneLab.exe</b>。<br/>3. 保留同目录的 resources、locales 等运行文件。不要只转发 exe，也不要在压缩包里直接运行。')
h('方式 B / 安装版')
add('运行 SceneLab-1.2.0-Setup-x64.exe，按向导选择安装位置。安装后可从桌面或开始菜单打开「SceneLab」。更新前先关闭旧版窗口并导出项目备份。')
table(['环境','说明'],[
...
add('当前没有账号同步、自动升级或任意方向剖切面。仿真预览不是照片级渲染，圆柱在二维概览中用外包框表达。v1.2 是当前实现功能的发布版，不代表原始 PRD 的全部条目已完成。', 'small')
h('反馈给项目维护人时')
...
def furniture(c, doc):
    ...
    c.setFont('CN',8);c.setFillColor(colors.HexColor(GRAY));c.drawString(56,32,'SCENELAB  v1.2   /   光学动捕部署与仿真')
    c.drawRightString(539,32,f'{doc.page:02} / 11');c.restoreState()

target = OUT/'SceneLab_v1.2_使用说明.pdf'
doc=SimpleDocTemplate(str(target),pagesize=(595.28,841.89),rightMargin=56,leftMargin=56,topMargin=48,bottomMargin=64,
    title='SceneLab v1.2 使用说明',author='SceneLab',subject='Windows x64 安装、部署、分析与项目交接')

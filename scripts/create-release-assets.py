...
page(1, 'SceneLab v1.4\n使用说明', '光学动捕相机部署与理论精度仿真工作台')
story.extend([SceneGraphic(),Spacer(1,18)])
add('从场地到方案，用空间分析辅助部署决策。', 'h2')
add('本指南面向首次使用的同事。软件在本机运行，内置相机参数库、三维场景编辑、覆盖与理论精度分析，并可输出技术报告。')
table(['从这里开始','页码'],[
    ['安装与第一次运行','02'],['认识界面，设置场地','03'],['相机库与相机部署','04'],['对象编辑与 Marker 尺寸','05'],['运行分析与读取热图','06'],['逐像素查看相机成像','07'],['技术报告、保存与交接','08'],['常见问题与计算边界','09'],['方管与 P3 IR','10'],['多选、分组与右键操作','11'],['导出技术报告包','12'],['自动部署与推荐方案','13'],['诊断、试验与 Kimi 助手','14'],
],[410,73])
story[-2].setStyle(TableStyle([('TOPPADDING',(0,0),(-1,-1),4),('BOTTOMPADDING',(0,0),(-1,-1),3)]))
add('版本 1.4.0 · Windows x64 · 发布日期 2026-09-23', 'small')

page(2, '安装与第一次运行', '选择安装版或免安装版即可。两者功能一致，均自带运行环境，不需要安装 Node.js，也不需要联网登录。')
h('方式 A / 免安装版')
add('1. 将 SceneLab-1.4.0-Windows-x64.zip <b>完整解压</b>到普通文件夹。<br/>2. 打开该文件夹，双击 <b>SceneLab.exe</b>。<br/>3. 保留同目录的 resources、locales 等运行文件。不要只转发 exe，也不要在压缩包里直接运行。')
h('方式 B / 安装版')
add('运行 SceneLab-1.4.0-Setup-x64.exe，按向导选择安装位置。安装后可从桌面或开始菜单打开「SceneLab」。更新前先关闭旧版窗口并导出项目备份。')
table(['环境','说明'],[
    ['操作系统','Windows 10 / 12，64 位（x64）；其他平台未验证。'],
...
h('三维视图导航')
add('<b>右键拖动</b>旋转视角；<b>中键拖动</b>平移；<b>滚轮</b>缩放。可切换透视、正交、俯视、前视、侧视。选中对象后按 <b>F</b> 聚焦；需要查看全场时使用「适配场景」。重叠物体可按住 <b>Alt</b> 连续点击来切换。')
...
table(['操作','提示'],[
    ...
    ['更新参数快照','可在左侧多选，再右键「更新为参数库版本」，按各自型号批量更新。'],
    ['结构吸附','吸附是一次性定位；后续移动结构不会自动带动相机。'],
],[110,373])
callout('演示参数与真实参数', 'M4 · Demo 及随包提供的 ABC-400 均为合成教学数据。真实项目请使用经过确认的镜头、分辨率、标定与工作距离参数。')

page(5, '对象编辑与 Marker 尺寸', '左侧 Shift+点击连续选择，Ctrl+点击增减；三维视图用 Ctrl 或 Shift+点击增减对象。多选后可批量编辑，详见第 11 页。')
table(['快捷键','效果'],[
...
add('边界不能被间距整除时，软件以等距单元中心覆盖整个场地，实际间距不超过输入值。热图最多显示 60,000 个三维单元，细网格按空间合并并连续铺满；合并区域保留最低覆盖与最差精度，统计使用全部有效采样点；计算上限为 1,200,000 点。', 'small')
table(['查看内容','如何理解'],[
    ['覆盖数量','每处采样点有多少个有效视点能观察到（单目 1 个，双目 2 个）；使用 ≥2、≥3、≥4 等指标比较方案。'],
...
add('当前没有账号同步、自动升级或任意方向剖切面。仿真预览不是照片级渲染，圆柱在二维概览中用外包框表达。v1.4 是当前实现功能的发布版，不代表原始 PRD 的全部条目已完成。', 'small')
h('反馈给项目维护人时')
add('附上：软件版本、复现步骤、预期与实际结果、问题截图，以及允许共享的项目文件。若涉及自定义型号，同时提供相机库 JSON。发送前检查文件中是否含有不适合共享的项目名称或参数。')

page(10, '方管与 P3 IR', '新增方管安装结构和 P3 IR 双目相机，并修复细采样热图出现分片空隙的问题。')
h('方管桁架：调整尺寸，安装相机')
add('在左侧「添加到场景」点击 <b>方管桁架</b>。默认长 4 m、宽 0.05 m、高 0.05 m；右侧「尺寸」可分别修改。长度沿局部 X 轴，通过旋转调整安装方向。三维模型是长方体外形，不模拟内部壁厚。')
add('选中相机，点击「吸附到结构」可贴近方管外侧；选中方管再打开「阵列部署」，使用「沿选中桁架 / 方管部署」。支持三维选择、复制粘贴、撤销重做及项目保存。吸附为一次性定位，之后移动结构不会自动带动相机；默认不参与遮挡，可在属性中启用。')
h('P3 IR：左右目独立仿真')
table(['参数','本版采用值'],[
 ['图像模式','仅 IR / Raw Grayscale；每目 2048 × 1536，120 fps'],
 ['视场与镜头','57.31° × 44.58°；4.2 ± 0.21 mm；F2.0'],
 ['双目基线','144 mm；左右目平行，沿相机局部水平轴对称分布'],
 ['机身','188 × 56 × 38.5 mm；484.3 g；CMOS 全局快门'],
 ['接口与供电','POE / 千兆以太网 / 双 RJ45 / 网络同步；待机 3.7 W、最高 15.7 W'],
],[95,388])
add('在相机库搜索 <b>P3</b>。已有相机库会自动加入新型号，保留原有编辑与删除记录。阵列部署或相机属性中选择「P3 · IR 双目」。设备数量计 1 台，覆盖与精度按左右两个有效视点计算；一目失效时，单台 P3 无法独立定位。')
add('相机预览可切换「左目 / 右目」，分别放大查看像素。空间点诊断分目显示，报告的二维成像章节也分别列出左右目。原始硬件参数在相机库「数据库来源」和备注中查看。')
callout('参数来源与兼容性', '按用户确认的分辨率计算，不采用误写的 1.6 MP / 1 MP。工作距离未设硬上限，图像点误差 0.1 px、最小 Marker 4 px、无畸变均为可编辑假设，结果不包含双目标定、匹配和照明误差。可打开 v1.0 项目；含新对象或双目参数的项目请使用 v1.2。')

page(11, '多选、分组与右键操作', '场景物体较多时，可从左侧列表按范围或类型选择，再统一整理和编辑。')
h('选择一批对象')
table(['操作','选择结果'],[
 ['Shift + 点击','先点击一个对象，再 Shift+点击另一个对象，连续选中可见列表中两者之间的行。'],
 ['Ctrl + 点击','增减单个对象；Ctrl+Shift+点击追加一个连续区间。'],
 ['顶部全选 / 相机 / 障碍物','选择全部对象或指定类型，包含已分组和已折叠的成员。左侧获得焦点后，Ctrl+A 也可全选。'],
 ['点击组名','选择整组；Ctrl+点击组名可追加另一组。左侧箭头用于折叠和展开。'],
],[128,355])
h('新建和整理分组')
add('多选后，在选中的行上点右键，选择 <b>「用所选对象新建组」</b>并输入名称。相机、障碍物、结构与 Marker 可以放进同一组。右键组名可以改名；选中成员后可移入已有组，或移出分组。')
add('分组随项目保存，复制时生成独立组。旧项目中的原有分组会显示为文件夹。分组为单层管理；选择成员后可一起移动和旋转，不建立父子安装约束。', 'small')
h('右键批量操作')
add('菜单提供复制、粘贴、创建副本、聚焦、隐藏 / 显示、锁定 / 解锁和删除。分组与批量编辑支持 Ctrl+Z 撤销、Ctrl+Y 重做。隐藏不等于停止参与仿真。')
add('<b>更新为参数库版本：</b>每台相机使用自己关联的库型号，位置与姿态保留。菜单显示可更新数量；锁定对象跳过删除、重新分组和参数更新。更改相机参数后请重新分析。')
h('重叠物体如何点选')
add('普通点击选择最前方的实体表面；按住 <b>Alt</b> 在同一位置连续点击，可在前后重叠物体之间循环切换。透明视锥和装饰边线不再抢占点击。很小的 Marker 可在左侧选中后按 F 聚焦。')

page(12, '导出技术报告包', '技术报告包保存当前方案的工程事实、分析摘要和图片，供其他软件读取；PDF 继续用于直接阅读。')
h('如何导出')
add('1. 完成场景和相机部署，点击 <b>「运行分析」</b>。<br/>2. 打开 <b>「技术报告」</b>，等待六张核心图片生成。<br/>3. 点击与 PDF 按钮并列的 <b>「导出技术报告包」</b>。<br/>4. 检查项目、方案、Revision 和有效性提示。<br/>5. 按需勾选附加内容，再点击「导出」。')
table(['包内文件','用途'],[
 ['manifest.json','入口清单，记录格式版本、软件版本和图片 role / view / file。'],
 ['project.json','仅当前方案：场地、设置、所有对象及每台相机完整参数快照。'],
 ['analysis.json','覆盖率、理论精度、达标率、采样点数量和耗时；不含完整体素数组。'],
 ['images/*.png','部署透视、覆盖俯视/透视、精度透视/前视/侧视六张图片，以及当前版本的手动截图。'],
],[128,355])
h('两个可选项，默认均关闭')
add('<b>各相机二维成像视图：</b>导出 camera_views/index.json 和每台相机的 PNG，双目分别输出左 / 右目。图片复用报告概览，最长边至多 2048 px；原始分辨率、Marker 像素坐标、投影尺寸和观测状态保存在 index 数据中，不能按概览符号数像素。')
add('<b>内部诊断信息：</b>额外导出问题类型、数量和代表点位置。仅建议内部技术评审使用。')
h('为什么导出按钮不能用')
add('若从未分析，先运行分析；若部署已修改，重新分析。当前分析对应图片缺失时，点击弹窗中的「重新生成报告图片」。导出期间更改方案或分析会终止本次导出；取消不会下载半成品。')
callout('文件格式与交接', '文件名为「项目名_方案名_R版本.scenelab-report」，内部采用 ZIP，可改后缀或用解压工具读取。包内不含 PDF，也不是可编辑项目备份；继续编辑仍需 .cameraplanner.json。整个导出过程在本机完成。')

page(13, '自动部署与推荐方案', '顶部「自动部署」会按约束生成候选，并使用现有仿真引擎评价。无需联网；原有阵列部署继续保留。')
h('1 / 设置条件')
add('场地、Marker 直径和精度阈值继承当前方案。输入最少有效视点数、覆盖目标和精度达标体积比例；相机型号可自动推荐或从当前参数库勾选。双目按 1 台设备、2 路观测计算，不以厂商标称精度代替仿真。')
table(['参数','选择方式'],[
 ['数量','指定数量、数量范围或自动搜索 8–64 台；范围模式最多 200 台。'],
 ['安装','周边桁架、顶装、混合、自由周边 / 三脚架、已有安装结构，或自动比较。'],
 ['层数与目标','自动 / 自定义层数；综合平衡、覆盖优先、精度优先、最少相机。'],
 ['空间权重','中心优先或全场均衡；可设置优先活动高度，仅影响搜索权重。'],
],[105,378])
h('2 / 搜索、预览和比较')
add('点击「开始搜索」。Worker 显示型号筛选、数量与层数搜索、局部优化、数量细化和正式精算进度，可随时取消。候选预览不会修改当前场景，支持旋转、缩放、视角切换及覆盖 / 精度热图。')
add('结果来自最多 5 个决赛候选的正式采样。不同目标选中相同方案时合并显示；查看 ≥2 / ≥3 / ≥4 覆盖、精度达标率、RMS 均值、P90 / P95、不可解样本和与其他候选的差值。')
h('3 / 应用为可编辑方案')
add('默认「创建新方案」保留原场景，也可替换当前全部相机或追加。可勾选创建推荐安装结构；只创建相机时，不保留指向未创建结构的挂载引用。目标方案采用规划场地及 Marker / 精度设置。')
callout('应用后仍可自由编辑', '相机与结构均为普通对象，可移动、旋转、复制、分组、删除。应用作为一次操作支持撤销 / 重做。应用后重新运行分析，尤其是追加相机或省略推荐结构时。')

page(14, '诊断、试验与 Kimi 助手', '先看覆盖与不可解区域，再比较精度。自动部署是可复现的模板与局部搜索，不保证全局最优。')
h('正式指标与最佳尝试')
add('粗筛采用自适应采样间距；界面显示实际值。最终推荐使用用户设定的正式间距，超过 120 万点会提示调整，不会以粗筛结果冒充正式分析。')
add('覆盖与精度达标率按全场未加权样本统计，排除实体内部。RMS 均值及 P90 / P95 只统计可解点；活动高度和中心权重不改变全场统计口径。')
add('目标未全部达到时显示「最佳尝试」，并保留推荐候选；安装容量、间距、层数或碰撞造成无法安装时才提示安装问题。单看中心少量点的高精度不能替代覆盖。')
h('诊断和放宽条件的实测对比')
add('诊断从最多 320 个均匀抽样点识别工作距离、Marker 过小、遮挡物、几何精度和高处覆盖限制。Marker 建议沿用原有投影尺寸公式，显示受大小限制样本的所需直径 P90，再向上取实用规格。')
add('最佳尝试最多进行 3 次独立精算，如增大 Marker、增加设备 / 层数、混合安装或换型号。前后差值来自真实仿真；这些试验可能超出当前约束，不会自动应用。')
h('可选 Kimi：先看草稿，再开始搜索')
add('在左侧展开「Kimi 助手」，填写自己的 API 密钥与账户可用模型，启用并保存。AI 默认关闭，离线规划完整可用。点击「解析为草稿」只发送需求文字、规划条件及型号名称 / ID；校验后先展示草稿，接受后仍可编辑。')
add('推荐解释仅整理正式结果的事实句子，不生成相机坐标、虚构指标或隐藏限制。API 错误不影响离线规划。浏览器开发版不保存 API 密钥，请在桌面版配置。')
callout('密钥仅属于这台电脑', '密钥经系统加密存储于本机用户数据目录，不进入工程、报告、源码或分发包。不要把整个用户数据目录分享给同事；每人可配置自己的密钥，也可随时关闭和移除。')

def furniture(c, doc):
    c.saveState()
    c.setStrokeColor(colors.HexColor('#D6E3DE')); c.setLineWidth(.6); c.line(56,48,539,48)
    c.setFont('CN',8);c.setFillColor(colors.HexColor(GRAY));c.drawString(56,32,'SCENELAB  v1.4   /   光学动捕部署与仿真')
    c.drawRightString(539,32,f'{doc.page:02} / 14');c.restoreState()

target = OUT/'SceneLab_v1.4_使用说明.pdf'
doc=SimpleDocTemplate(str(target),pagesize=(595.28,841.89),rightMargin=56,leftMargin=56,topMargin=48,bottomMargin=64,
    title='SceneLab v1.4 使用说明',author='SceneLab',subject='Windows x64 安装、部署、分析与项目交接')
